...
const toggleLike = async (req, res) => {
  try {
    ...
    // Primero asegurar que el comentario tenga los campos likes y dislikes
    await Producto.updateOne(
      { 'comentarios._id': comentarioId, 'comentarios.likes': { $exists: false } },
      { $set: { 'comentarios.$.likes': [] } }
    );

    await Producto.updateOne(
      { 'comentarios._id': comentarioId, 'comentarios.dislikes': { $exists: false } },
      { $set: { 'comentarios.$.dislikes': [] } }
    );

    // Recargar el producto para obtener los campos actualizados
    producto = await Producto.findOne({ 'comentarios._id': comentarioId });
    let comentarioConCampos = producto.comentarios.id(comentarioId);

    // Verificar si el usuario ya dio like
    const likes = comentarioConCampos.likes || [];
    ...
    const productoFinal = await Producto.findOne({ 'comentarios._id': comentarioId });
    const comentarioActualizado = productoFinal.comentarios.id(comentarioId);
...
const toggleDislike = async (req, res) => {
  try {
    ...
    // Recargar el producto para obtener los campos actualizados
    producto = await Producto.findOne({ 'comentarios._id': comentarioId });
    let comentarioConCamposDislike = producto.comentarios.id(comentarioId);

    // Verificar si el usuario ya dio dislike
    const dislikes = comentarioConCamposDislike.dislikes || [];
    ...
    // Obtener el comentario actualizado para devolver los conteos correctos
    const productoFinalDislike = await Producto.findOne({ 'comentarios._id': comentarioId });
    const comentarioActualizadoDislike = productoFinalDislike.comentarios.id(comentarioId);

    // Asegurar que los arrays existen y tienen valores por defecto
    if (!comentarioActualizadoDislike.likes || comentarioActualizadoDislike.likes === undefined) {
      comentarioActualizadoDislike.likes = [];
    }
    if (!comentarioActualizadoDislike.dislikes || comentarioActualizadoDislike.dislikes === undefined) {
      comentarioActualizadoDislike.dislikes = [];
    }

    console.log('Comentario actualizado:', {
      id: comentarioActualizadoDislike._id,
      likes: comentarioActualizadoDislike.likes,
      dislikes: comentarioActualizadoDislike.dislikes,
      likesLength: comentarioActualizadoDislike.likes ? comentarioActualizadoDislike.likes.length : 0,
      dislikesLength: comentarioActualizadoDislike.dislikes ? comentarioActualizadoDislike.dislikes.length : 0
    });

    const userLiked = comentarioActualizadoDislike.likes && comentarioActualizadoDislike.likes.some(id => id.toString() === usuarioId.toString());
    const userDisliked = comentarioActualizadoDislike.dislikes && comentarioActualizadoDislike.dislikes.some(id => id.toString() === usuarioId.toString());

    const response = {
      mensaje: `Dislike ${accion === 'added' ? 'agregado' : 'removido'} exitosamente`,
      likes: comentarioActualizadoDislike.likes ? comentarioActualizadoDislike.likes.length : 0,
      dislikes: comentarioActualizadoDislike.dislikes ? comentarioActualizadoDislike.dislikes.length : 0,
      userLiked: userLiked || false,
      userDisliked: userDisliked || false
    };
